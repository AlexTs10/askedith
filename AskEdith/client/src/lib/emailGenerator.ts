...
function extractUserInfo(answers: WizardAnswers) {
      ...
      if (answers.q14.trim().startsWith("{")) {
        ...
        if (!email || !email.includes("@")) {
          ...
          for (const key of Object.keys(contactInfo)) {
            if (
              typeof contactInfo[key] === "string" &&
              contactInfo[key].includes("@") &&
              contactInfo[key].includes(".")
            ) {
              console.log(
                `Found potential email in ${key} field:`,
                contactInfo[key],
              );
        ...
        console.warn(
          "Contact info does not start with {, raw data:",
          answers.q14,
        );
      }
    } else {
  ...
  if (!email || !email.includes("@") || !email.includes(".")) {
    console.warn(
      "Invalid or missing email after all extraction attempts - will need fallback",
    );
...
I'm hoping to connect with you to discuss how your services might fit our needs or what resources we could apply for that could help us as we care for our ${relation}.

May we connect at a time that is convenient for you for a short conversation? I'm available most days and can adjust my schedule to accommodate yours. I look forward to speaking with you.
